...
public class IRailExample {
	...
	public static void main(String[] argv) throws Exception {
		...
		for (Connection connection : connections) {
			...
			System.out.printf("DEPARTURE [%2d:%02d %-22s %-3s %2s +%02dH%02d] -> ", cal.get(Calendar.HOUR_OF_DAY),
					cal.get(Calendar.MINUTE), departure.getStation().getName(), departure.getVehicle().getType(),
					departure.getPlatform(), (departure.getDelay() / 60) / 60, (departure.getDelay() / 60) % 60);

			cal.setTime(arrival.getTime());
			System.out.printf("ARRIVAL [%2d:%02d %-22s %-3s %2s]\n", cal.get(Calendar.HOUR_OF_DAY),
					cal.get(Calendar.MINUTE), arrival.getStation().getName(), arrival.getVehicle().getType(),
					arrival.getPlatform());
