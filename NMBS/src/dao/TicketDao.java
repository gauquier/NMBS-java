...
public class TicketDao {
		...
		try {
			if (rs.next()) {
				DateFormat dt = new SimpleDateFormat("dd-MM-yyyy");// aan te
																	...
																	// wijzing
																	// boris
				ticket = new Ticket(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getInt(5),
						rs.getDouble(6), VerkoopType.VerkoopTypeCasting(rs.getString(7)), rs.getDouble(8), rs.getInt(9),
						rs.getInt(10), dt.parse(rs.getString(11)), dt.parse(rs.getString(12)),
						dt.parse(rs.getString(13)));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static ArrayList<Ticketstatistiek> getTicketstatistieken() {
		/*
		 * SELECT verkoopDatum, COUNT(verkoopDatum) FROM Ticket GROUP BY
		 * verkoopDatum LIMIT 5;
		 */
		...
		try {
			while (rs.next()) {
				...
				// ticketstatistieken.add(new
				// Ticketstatistiek(tijd,rs.getInt(2)));
