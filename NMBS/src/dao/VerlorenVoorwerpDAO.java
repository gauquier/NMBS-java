...
public class VerlorenVoorwerpDAO {
	...
	public static int insertVerlorenVoorwerp(VerlorenVoorwerp verlorenVoorwerp, int stationId) {
		if (VerlorenVoorwerpDAO.getId(verlorenVoorwerp) == 0) {
		...
		return VerlorenVoorwerpDAO.getId(verlorenVoorwerp);
	}
	...
	public static ArrayList<VerlorenVoorwerp> getVerlorenVoorwerpByStation(int stationId) {
		...
		try {
			while (rs.next()) {
				arr.add(new VerlorenVoorwerp(rs.getInt(1), rs.getString(3), df.parse(rs.getString(4)),
						rs.getBoolean(5)));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public static void main(String[] args) {
		/*
		 * @SuppressWarnings("deprecation") VerlorenVoorwerp v = new
		 * VerlorenVoorwerp(-1, "gsm", new java.sql.Date(11,11,2016), false);
		 * VerlorenVoorwerp s = new VerlorenVoorwerp(-1, "headPhone", new
		 * java.sql.Date(1, 2, 2013), true); VerlorenVoorwerpDAO vvd = new
		 * VerlorenVoorwerpDAO(); vvd.insertVerlorenVoorwerp(s, 4);
		 * 
		 * VerlorenVoorwerp za = vvd.getVerlorenVoorwerp(35);
		 * System.out.println(za.toString()); ArrayList<VerlorenVoorwerp> ve =
		 * vvd.getVerlorenVoorwerpByStation(2);
		 * 
		 ...
		 * System.out.println(test.toString()); // Create an instance of
		 * SimpleDateFormat used for formatting // the string representation of
		 * date (month/day/year) DateFormat df = new
		 * SimpleDateFormat("dd-MM-yyyy");
		 * 
		 * // Get the date today using Calendar object. Date today =
		 * test.getDate(); // Using DateFormat format method we can create a
		 * string // representation of a date with the defined format. String
		 * reportDate = df.format(today);
		 * 
		 * // Print what date is today! //System.out.println("Report Date: " +
		 * reportDate); //verlorenVoorwerpDAO.verlorenVoowerpUpdate(57, test);
		 */
