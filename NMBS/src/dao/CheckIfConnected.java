...
public class CheckIfConnected {
	...
	public static boolean checkIfConnected() {
		...
		Process p1;
		try {
			//ik doe de check op deze manier om een out of bounds exception te verkomen bij kortere namen (bv. "windows 10" vs "linux")
			if (os.length() > 6) {
				if (os.substring(0, 7).equals("windows")) {
					windows = true;
				}
			}
			if (os.length() > 5) {//voorkom out of bounds exception bij kortere naam
				if (os.substring(0, 6).equals("linux")) {
					linux = true;
				}
			}
			if (os.length() > 2) {//voorkom out of bounds exception bij kortere naam
